...
function [A43, A44, A45, A46] = LearnAndUpdate(AgentConfigs, Psq, PsqVariance, Agent, AgentNo, i, z)
	A43=Agent(i, 43, z);
	%fprintf('LearnAndUpdate-current sim(%d), itr(%d)\n', z, i);	
	%update the last squeezed profit first for the current iteration
	[A44, A45, A46] = UpdateLastSqueezedData(Agent, i, z);
	%fprintf('current (%d) itr-data: Psq=%d, 1stsqueezedprofit=%f, 2ndsqueezedprofit=%f, diff=%f\n', i, Agent(i,43,z), Agent(i,44,z), Agent(i,45,z), Agent(i,46,z));
	Agent(i,44,z) = A44;
	Agent(i,45,z) = A45;
	Agent(i,46,z) = A46;
